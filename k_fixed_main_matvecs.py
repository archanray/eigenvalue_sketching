...
from src.approximator import EigenGamesUnloadedForEigs as egue
...
if "bki_adp_Q" in approx_mthds:
    print("Approximator: Block Krylov Adaptive: Q with k =10")
    all_ks = [10]#list(range(16,80,50))
    all_qs = list(range(0,14,2))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
      ...
      for j in range(len(all_qs)):
            ...
            count += 1
    save_dict["bki_adp_Q_10"] = []
    save_dict["bki_adp_Q_10"].append(avg_errors)  #0
    save_dict["bki_adp_Q_10"].append(p20_errors)  #1
    save_dict["bki_adp_Q_10"].append(p80_errors)  #2
    save_dict["bki_adp_Q_10"].append(avg_lies)    #3 
    save_dict["bki_adp_Q_10"].append(p20_lies)    #4
    save_dict["bki_adp_Q_10"].append(p80_lies)    #5
    save_dict["bki_adp_Q_10"].append(matvecs_all) #6
######################################################################################

########################### Approximator -- bki_adp ##################################
if "bki_adp_Q" in approx_mthds:
    print("Approximator: Block Krylov Adaptive: Q with k =40")
    all_ks = [40]#list(range(16,80,50))
    all_qs = list(range(0,10,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p80_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    avg_lies = np.zeros(len(all_ks)*len(all_qs))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
      ...
      for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["bki_adp_Q_40"] = []
    save_dict["bki_adp_Q_40"].append(avg_errors)  #0
    save_dict["bki_adp_Q_40"].append(p20_errors)  #1
    save_dict["bki_adp_Q_40"].append(p80_errors)  #2
    save_dict["bki_adp_Q_40"].append(avg_lies)    #3 
    save_dict["bki_adp_Q_40"].append(p20_lies)    #4
    save_dict["bki_adp_Q_40"].append(p80_lies)    #5
    save_dict["bki_adp_Q_40"].append(matvecs_all) #6
######################################################################################

########################### Approximator -- bki_adp ##################################
if "bki_adp_Q" in approx_mthds:
    print("Approximator: Block Krylov Adaptive: Q with k =80")
    all_ks = [80]#list(range(16,80,50))
    all_qs = list(range(0,8,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p80_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    avg_lies = np.zeros(len(all_ks)*len(all_qs))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
      ...
      for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["bki_adp_Q_80"] = []
    save_dict["bki_adp_Q_80"].append(avg_errors)  #0
    save_dict["bki_adp_Q_80"].append(p20_errors)  #1
    save_dict["bki_adp_Q_80"].append(p80_errors)  #2
    save_dict["bki_adp_Q_80"].append(avg_lies)    #3 
    save_dict["bki_adp_Q_80"].append(p20_lies)    #4
    save_dict["bki_adp_Q_80"].append(p80_lies)    #5
    save_dict["bki_adp_Q_80"].append(matvecs_all) #6
######################################################################################

########################### Approximator -- bki_adp ##################################
if "bki_adp_Z" in approx_mthds:
    print("Approximator: Block Krylov Adaptive: Z with k = 10")
    all_ks = [10]#list(range(16,80,50))
    all_qs = list(range(0,14,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
        ...
        for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["bki_adp_Z_10"] = []
    save_dict["bki_adp_Z_10"].append(avg_errors)
    save_dict["bki_adp_Z_10"].append(p20_errors)
    save_dict["bki_adp_Z_10"].append(p80_errors)
    save_dict["bki_adp_Z_10"].append(avg_lies)
    save_dict["bki_adp_Z_10"].append(p20_lies)
    save_dict["bki_adp_Z_10"].append(p80_lies)
    save_dict["bki_adp_Z_10"].append(matvecs_all)
######################################################################################

########################### Approximator -- bki_adp ##################################
if "bki_adp_Z" in approx_mthds:
    print("Approximator: Block Krylov Adaptive: Z with k = 40")
    all_ks = [40]#list(range(16,80,50))
    all_qs = list(range(0,10,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
        ...
        for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["bki_adp_Z_40"] = []
    save_dict["bki_adp_Z_40"].append(avg_errors)
    save_dict["bki_adp_Z_40"].append(p20_errors)
    save_dict["bki_adp_Z_40"].append(p80_errors)
    save_dict["bki_adp_Z_40"].append(avg_lies)
    save_dict["bki_adp_Z_40"].append(p20_lies)
    save_dict["bki_adp_Z_40"].append(p80_lies)
    save_dict["bki_adp_Z_40"].append(matvecs_all)
######################################################################################

########################### Approximator -- bki_adp ##################################
if "bki_adp_Z" in approx_mthds:
    print("Approximator: Block Krylov Adaptive: Z with k = 80")
    all_ks = [80]#list(range(16,80,50))
    all_qs = list(range(0,8,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
        ...
        for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["bki_adp_Z_80"] = []
    save_dict["bki_adp_Z_80"].append(avg_errors)
    save_dict["bki_adp_Z_80"].append(p20_errors)
    save_dict["bki_adp_Z_80"].append(p80_errors)
    save_dict["bki_adp_Z_80"].append(avg_lies)
    save_dict["bki_adp_Z_80"].append(p20_lies)
    save_dict["bki_adp_Z_80"].append(p80_lies)
    save_dict["bki_adp_Z_80"].append(matvecs_all)
######################################################################################

########################## Approximator -- eg_unldd ##################################
if "eg_unldd" in approx_mthds:
    print("Approximator: Eigengames Unloaded with k=10")
    all_ks = [10]#list(range(16,80,50))
    all_qs = list(range(0,14,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
        ...
        for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["eg_unldd_10"] = []
    save_dict["eg_unldd_10"].append(avg_errors)
    save_dict["eg_unldd_10"].append(p20_errors)
    save_dict["eg_unldd_10"].append(p80_errors)
    save_dict["eg_unldd_10"].append(avg_lies)
    save_dict["eg_unldd_10"].append(p20_lies)
    save_dict["eg_unldd_10"].append(p80_lies)
    save_dict["eg_unldd_10"].append(matvecs_all)
######################################################################################

########################## Approximator -- eg_unldd ##################################
if "eg_unldd" in approx_mthds:
    print("Approximator: Eigengames Unloaded with k=40")
    all_ks = [40]#list(range(16,80,50))
    all_qs = list(range(0,10,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
        ...
        for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["eg_unldd_40"] = []
    save_dict["eg_unldd_40"].append(avg_errors)
    save_dict["eg_unldd_40"].append(p20_errors)
    save_dict["eg_unldd_40"].append(p80_errors)
    save_dict["eg_unldd_40"].append(avg_lies)
    save_dict["eg_unldd_40"].append(p20_lies)
    save_dict["eg_unldd_40"].append(p80_lies)
    save_dict["eg_unldd_40"].append(matvecs_all)
######################################################################################

########################## Approximator -- eg_unldd ##################################
if "eg_unldd" in approx_mthds:
    print("Approximator: Eigengames Unloaded with k=80")
    all_ks = [80]#list(range(16,80,50))
    all_qs = list(range(0,8,2))
    avg_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    p20_errors = np.zeros((len(all_ks)*len(all_qs), len(search_ranks)))
    ...
    for i in tqdm(range(len(all_ks)), position=0):
        ...
        for j in range(len(all_qs)):
            ...
            matvecs_all[count] = matvecs
            count += 1
    save_dict["eg_unldd_80"] = []
    save_dict["eg_unldd_80"].append(avg_errors)
    save_dict["eg_unldd_80"].append(p20_errors)
    save_dict["eg_unldd_80"].append(p80_errors)
    save_dict["eg_unldd_80"].append(avg_lies)
    save_dict["eg_unldd_80"].append(p20_lies)
    save_dict["eg_unldd_80"].append(p80_lies)
    save_dict["eg_unldd_80"].append(matvecs_all)
######################################################################################
...
with open("results/"+dataset_name+adder+"_kspec"+".pkl", "wb") as f:
